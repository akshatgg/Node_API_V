...
export default function SuperadminCheck(req: Request, res: Response, next: NextFunction) {
   ...
   const superuser =user.userType==="superadmin";
    ...
    if(!superuser) {
        return res.status(401).send({ success: false, message: 'Unauthorized Access. You must be an Superadmin to perform this operation.' });
    }
