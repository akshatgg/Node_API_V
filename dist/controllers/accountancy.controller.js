"use strict";
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TransactionController = exports.JournalEntryController = exports.LedgerController = void 0;
var __1 = require("..");
var LedgerController = /** @class */ (function () {
    function LedgerController() {
    }
    LedgerController.createLedger = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var userId, _a, ledgerName, ledgerType, openingBalance, ledger, error_1;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        _b.trys.push([0, 2, , 3]);
                        userId = req.user.id;
                        _a = req.body, ledgerName = _a.ledgerName, ledgerType = _a.ledgerType, openingBalance = _a.openingBalance;
                        return [4 /*yield*/, __1.prisma.ledger.create({
                                data: { ledgerName: ledgerName, ledgerType: ledgerType, openingBalance: openingBalance, userId: userId, },
                            })];
                    case 1:
                        ledger = _b.sent();
                        return [2 /*return*/, res.json(ledger)];
                    case 2:
                        error_1 = _b.sent();
                        console.log(error_1);
                        return [2 /*return*/, res.status(500).json({ success: false, message: "Error creating ledger" })];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    LedgerController.getLedgers = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var userId, ledgers, error_2;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        userId = req.user.id;
                        return [4 /*yield*/, __1.prisma.ledger.findMany({
                                where: {
                                    userId: userId,
                                }
                            })];
                    case 1:
                        ledgers = _a.sent();
                        return [2 /*return*/, res.status(200).json({ success: true, ledgers: ledgers })];
                    case 2:
                        error_2 = _a.sent();
                        return [2 /*return*/, res.status(500).json({ success: false, message: "Error fetching ledgers" })];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    LedgerController.getLedgerById = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var userId, ledgerId, ledger, error_3;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        userId = req.user.id;
                        ledgerId = req.params.id;
                        return [4 /*yield*/, __1.prisma.ledger.findFirst({ where: { id: ledgerId, userId: userId } })];
                    case 1:
                        ledger = _a.sent();
                        if (!ledger) {
                            return [2 /*return*/, res.status(404).json({ success: false, message: 'Ledger not found' })];
                        }
                        return [2 /*return*/, res.status(200).json({ success: true, ledger: ledger })];
                    case 2:
                        error_3 = _a.sent();
                        return [2 /*return*/, res.status(500).json({ success: false, message: 'Internal server error' })];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    LedgerController.getLedgerByPartyId = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var userId, date, currentYear, currentMonth, partyId, _a, year, month, ledger, error_4;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        _b.trys.push([0, 2, , 3]);
                        userId = req.user.id;
                        date = new Date();
                        currentYear = date.getFullYear();
                        currentMonth = date.getMonth();
                        partyId = req.params.id;
                        _a = req.query, year = _a.year, month = _a.month;
                        return [4 /*yield*/, __1.prisma.ledger.findFirst({ where: {
                                    partyId: partyId,
                                    year: year ? parseInt(year) : currentYear,
                                    month: month ? parseInt(month) : currentMonth,
                                    userId: userId,
                                } })];
                    case 1:
                        ledger = _b.sent();
                        if (!ledger) {
                            return [2 /*return*/, res.status(404).json({ success: false, message: 'Ledger not found' })];
                        }
                        return [2 /*return*/, res.status(200).json({ success: true, ledger: ledger })];
                    case 2:
                        error_4 = _b.sent();
                        return [2 /*return*/, res.status(500).json({ success: false, message: 'Internal server error' })];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    return LedgerController;
}());
exports.LedgerController = LedgerController;
var JournalEntryController = /** @class */ (function () {
    function JournalEntryController() {
    }
    JournalEntryController.createJournalEntry = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var userId_1, _a, entryDate, description, transactions, journalEntry, error_5;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        _b.trys.push([0, 2, , 3]);
                        userId_1 = req.user.id;
                        _a = req.body, entryDate = _a.entryDate, description = _a.description, transactions = _a.transactions;
                        return [4 /*yield*/, __1.prisma.journalEntry.create({
                                data: {
                                    entryDate: entryDate,
                                    description: description,
                                    userId: userId_1,
                                    transactions: {
                                        create: transactions.map(function (transaction) { return (__assign({ userId: userId_1 }, transaction)); })
                                    }
                                },
                            })];
                    case 1:
                        journalEntry = _b.sent();
                        return [2 /*return*/, res.status(200).json({ success: true, journalEntry: journalEntry })];
                    case 2:
                        error_5 = _b.sent();
                        return [2 /*return*/, res.status(500).json({ success: false, message: "Error creating journal entry" })];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    JournalEntryController.getJournalEntries = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var userId, journalEntries, error_6;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        userId = req.user.id;
                        return [4 /*yield*/, __1.prisma.journalEntry.findMany({
                                where: {
                                    userId: userId,
                                }
                            })];
                    case 1:
                        journalEntries = _a.sent();
                        return [2 /*return*/, res.status(200).json({ success: true, journalEntries: journalEntries })];
                    case 2:
                        error_6 = _a.sent();
                        return [2 /*return*/, res.status(500).json({ success: false, message: "Error fetching journal entries" })];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    return JournalEntryController;
}());
exports.JournalEntryController = JournalEntryController;
var TransactionController = /** @class */ (function () {
    function TransactionController() {
    }
    TransactionController.getTransactions = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var userId, ledgerId, transactions, error_7;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        userId = req.user.id;
                        ledgerId = req.body.ledgerId;
                        return [4 /*yield*/, __1.prisma.transaction.findMany({
                                where: {
                                    userId: userId,
                                    ledgerId: ledgerId,
                                }
                            })];
                    case 1:
                        transactions = _a.sent();
                        return [2 /*return*/, res.status(200).json({ success: true, transactions: transactions })];
                    case 2:
                        error_7 = _a.sent();
                        return [2 /*return*/, res.status(500).json({ success: false, message: "Error fetching transactions" })];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    return TransactionController;
}());
exports.TransactionController = TransactionController;
//# sourceMappingURL=accountancy.controller.js.map